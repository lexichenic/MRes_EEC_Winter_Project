...
setwd("C:/Users/Lexi C/Desktop/Winter Project/Data collection & analysis")
Data <- read_excel("Import_data.xlsx")
```
...
hist(Data$Effect_size,xlab = "Effect Size", main = "All effect sizes")
```
...
taxa<-c("Acrocephalus arundinaceus",
        ...
        "Troglodytes aedon")
taxa<-tnrs_match_names(taxa,context_name = "Animals")
...
Phylomatrix<-vcv(tree,cor=T)
Data[,"Phylogeny"]<-tolower(Data$Phylogeny)
```
...
sigma_total <- 1.556943e-10+2.058583e-02+1.273269e-02+5.529262e-03+9.222487e-05
sigma_total # 0.03894001
I2species<- 2.058583e-02/sigma_total
I2species # 0.528655
I2between_study<-1.273269e-02/sigma_total
I2between_study # 0.3269822
I2within_study<-5.529262e-03/sigma_total
I2within_study # 0.1419944
H2phylogenetic_heritability<- 1.556943e-10/( 1.556943e-10+2.058583e-02+1.273269e-02+5.529262e-03)
H2phylogenetic_heritability # 4.007804e-09

# creating a data.frame with the meta-analytic mean and heterogeneity estimates
table.model.r <- data.frame(n=length(unique(Data$Author)),
                            k=nrow(Data),
...
table.model.r.gt <- table.model.r %>% 
  ...
  cols_label(n=md("**n**"),
             ...
             CI=md("**95% CI**"),
             PI=md("**95% PI**"),
             I2_Data_ID=md("***I*<sup>2</sup><sub>wtihin-study</sub>\n(%)**"),
             I2_Author=md("***I*<sup>2</sup><sub>between-study</sub>\n(%)**"),
             I2_Species=md("***I*<sup>2</sup><sub>species</sub>\n(%)**"),
             I2_Phylogeny=md("***I*<sup>2</sup><sub>phylogeny</sub>\n(%)**")
  ) %>%
...
g3<-facet_plot(tree,panel="Forest plot",data = d1,geom=geom_point,aes(x=Effect_size),stat= 'identity') + ggtree::xlim_expand(xlim=c(-1,1.5),panel="Forest plot")
g3
g4<-g3 + xlim_tree(25) + xlim_expand(xlim=c(-0.5,0.5), panel = "Forest plot") +
  theme_tree2()
g4
g5<-facet_plot(g4,geom = geom_errorbarh, data=d1,aes(x=Effect_size,xmin=Lower_CI,xmax=Upper_CI),panel = "Forest plot")
g5
d=data.frame(x=c(-1,-0.5,0,0.5,1,1.5),.panel='Forest plot')
g6<-g5+geom_vline(data=d,aes(xintercept=x),color = "grey")
d2=data.frame(x=0,.panel='Forest plot')
g7<-g6+geom_vline(data=d2,aes(xintercept=x),linetype = "dotted")
g7
ggsave(g7,file="forest3.pdf")
```

```{r}
funnel(model,yaxis = "seinv",xlab = "Effect size")
```
